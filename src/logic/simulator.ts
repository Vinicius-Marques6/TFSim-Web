import type { Instruction, InstructionStatus, ReservationStationEntry, SimulationState } from "./types";
import { Opcode } from "./types";

// Configuração das unidades funcionais e seus tempos de execução
const EXECUTION_TIMES: Record<Opcode, number> = {
  [Opcode.LW]: 2,
  [Opcode.SW]: 2,
  [Opcode.ADD]: 2,
  [Opcode.SUB]: 2,
  [Opcode.MUL]: 10,
  [Opcode.DIV]: 40,
};

// Configuração das Estações de Reserva
export const RESERVATION_STATIONS_CONFIG: { name: string, type: Opcode[], count: number }[] = [
    { name: 'Add', type: [Opcode.ADD, Opcode.SUB], count: 3 },
    { name: 'Mul', type: [Opcode.MUL, Opcode.DIV], count: 2 },
    { name: 'Load', type: [Opcode.LW, Opcode.SW], count: 2 },
];

export class TomasuloSimulator {
  private state: SimulationState;
  private instructionStatus: InstructionStatus[];

  constructor(instructions: Instruction[], registers: string[], memory: Record<string, number>) {
    this.state = this.initializeState(instructions, registers, memory);
    this.instructionStatus = instructions.map(instruction => ({
      instruction,
      issue: null,
      executionStart: null,
      executionEnd: null,
      writeResult: null,
    }));
  }

  /**
   * Inicializa o estado do simulador.
   */
  private initializeState(instructions: Instruction[], registerNames: string[], memory: Record<string, number>): SimulationState {
    const registerFile = Object.fromEntries(
      registerNames.map(name => [name, { value: 0, qi: null }])
    );

    const reservationStations: ReservationStationEntry[] = [];
    RESERVATION_STATIONS_CONFIG.forEach(config => {
        for (let i = 1; i <= config.count; i++) {
            reservationStations.push({
                name: `${config.name}${i}`,
                busy: false,
                op: null,
                vj: null,
                vk: null,
                qj: null,
                qk: null,
                dest: null,
                executionTimeLeft: null,
                address: null,
            });
        }
    });

    return {
      clock: 0,
      instructions,
      reservationStations,
      registerFile,
      memory,
    };
  }

  /**
   * Avança a simulação em um ciclo de clock.
   */
  public nextStep() {
    this.state.clock++;

    // A ordem é importante: Escrita, Execução, Emissão para simular o paralelismo
    this.writeResultPhase();
    this.executePhase();
    this.issuePhase();

    // Retorna uma cópia profunda para garantir que o React detecte a mudança
    return JSON.parse(JSON.stringify(this.state));
  }

  /**
   * Fase de Emissão (Issue)
   * - Pega a próxima instrução da fila.
   * - Encontra uma Estação de Reserva (ER) vaga do tipo correto.
   * - Se não houver ER vaga, para (hazard estrutural).
   * - Copia os operandos para a ER. Se um operando não estiver pronto,
   *   rastreia a ER que o produzirá (renomeação de registradores).
   * - Atualiza o arquivo de registradores para apontar para a nova ER.
   */
  private issuePhase() {
    if (this.state.instructions.length === 0) {
      return; // Fila de instruções vazia
    }

    const instruction = this.state.instructions[0];
    const stationTypeConfig = RESERVATION_STATIONS_CONFIG.find(c => c.type.includes(instruction.op));

    if (!stationTypeConfig) {
      console.error(`Operação ${instruction.op} não suportada.`);
      this.state.instructions.shift(); // Remove a instrução inválida
      return;
    }

    const station = this.state.reservationStations.find(rs =>
      rs.name.startsWith(stationTypeConfig.name) && !rs.busy
    );

    if (!station) {
      return; // Hazard estrutural: nenhuma estação de reserva disponível
    }

    // Ocupa a estação
    station.busy = true;
    station.op = instruction.op;

    console.log(`Emissão da instrução: ${instruction.op} para a estação ${station.name}`);

    // Lógica para tratar diferentes tipos de instrução
    switch (instruction.op) {
      case Opcode.ADD:
      case Opcode.SUB:
      case Opcode.MUL:
      case Opcode.DIV: {
        // Operações R-Type (ADD Rd, Rs, Rt)
        // operand1 = Rs, operand2 = Rt
        const regSource1 = this.state.registerFile[instruction.operand1];
        if (regSource1 && regSource1.qi) {
          station.qj = regSource1.qi;
        } else {
          station.vj = regSource1 ? regSource1.value : 0;
          station.qj = null;
        }

        const regSource2 = this.state.registerFile[instruction.operand2];
        if (regSource2 && regSource2.qi) {
          station.qk = regSource2.qi;
        } else {
          station.vk = regSource2 ? regSource2.value : 0;
          station.qk = null;
        }
        
        // Atualiza o registrador de destino
        if (this.state.registerFile[instruction.dest]) {
            this.state.registerFile[instruction.dest].qi = station.name;
        }
        station.dest = instruction.dest;
        break;
      }
      case Opcode.LW:
      case Opcode.SW: {
        // Formato: OP Rd, offset(Rs)
        // operand1 = offset, operand2 = Rs (registrador base)
        // dest = Rd para LW, Rt para SW (registrador com o dado)

        // Pega o valor do registrador base (Rs)
        const baseRegister = this.state.registerFile[instruction.operand2];
        if (baseRegister && baseRegister.qi) {
          station.qj = baseRegister.qi; // Dependência do registrador base
        } else {
          // O valor do registrador base é colocado em Vj para o cálculo do endereço
          station.vj = baseRegister ? baseRegister.value : 0;
          station.qj = null;
        }

        // O offset é tratado como um valor imediato e armazenado em address
        station.address = parseInt(instruction.operand1, 10);
        station.vk = null;
        station.qk = null;

        if (instruction.op === Opcode.LW) {
            // Atualiza o registrador de destino (Rd)
            if (this.state.registerFile[instruction.dest]) {
                this.state.registerFile[instruction.dest].qi = station.name;
            }
            station.dest = instruction.dest;
        } else { // SW
            // Para SW, o registrador de destino da instrução (dest) é a fonte do dado
            const sourceRegister = this.state.registerFile[instruction.dest];
            if (sourceRegister && sourceRegister.qi) {
                // O valor a ser armazenado depende de outra estação
                station.qk = sourceRegister.qi;
            } else {
                // O valor a ser armazenado está pronto
                station.vk = sourceRegister ? sourceRegister.value : 0;
                station.qk = null;
            }
            // SW não tem registrador de destino no register file, o destino é a memória
            station.dest = null; 
        }
        break;
      }
    }

    // Remove a instrução da fila de emissão
    this.state.instructions.shift();

    // Atualiza o status da instrução
    const issuedInstructionStatus = this.instructionStatus.find(s => s.instruction === instruction && s.issue === null);
    if (issuedInstructionStatus) {
      issuedInstructionStatus.issue = this.state.clock;
    }
  }

  /**
   * Fase de Execução (Execute)
   * - Itera sobre as ERs ocupadas.
   * - Se uma ER tem todos os operandos (Vj, Vk prontos), inicia a execução.
   * - Para LW/SW, primeiro calcula o endereço de memória.
   * - Para outras operações, começa a contagem de tempo de execução.
   * - Se uma ER já está em execução, decrementa seu contador de tempo.
   */
  private executePhase() {
    for (const station of this.state.reservationStations) {
      if (!station.busy || station.qj !== null || station.qk !== null) {
        // Estação não está pronta para executar (ou já terminou e aguarda escrita)
        continue;
      }

      // Se a execução ainda não começou, inicie-a
      if (station.executionTimeLeft === null) {
        switch (station.op) {
          case Opcode.LW:
          case Opcode.SW: {
            // Para LW/SW, o primeiro passo é calcular o endereço
            // Assumimos que o cálculo do endereço leva 1 ciclo.
            station.address = (station.vj ?? 0) + (station.address ?? 0);
            station.executionTimeLeft = EXECUTION_TIMES[station.op];
            break;
          }
          case Opcode.ADD:
          case Opcode.SUB:
          case Opcode.MUL:
          case Opcode.DIV: {
            station.executionTimeLeft = EXECUTION_TIMES[station.op];
            break;
          }
        }
      }

      // Se a estação está em processo de execução, decremente o contador
      if (station.executionTimeLeft !== null && station.executionTimeLeft > 0) {
        station.executionTimeLeft--;
      }
    }
  }

  /**
   * Fase de Escrita de Resultado (Write Result)
   * - Encontra ERs que terminaram a execução (tempo = 0).
   * - Simula o CDB: apenas uma ER pode escrever por ciclo.
   * - Calcula o resultado. LW/SW acessam a memória.
   * - Transmite o resultado para o arquivo de registradores e outras ERs.
   * - Libera a ER que escreveu o resultado.
   */
  private writeResultPhase() {
    // Encontra a primeira estação pronta para escrever (simulação do CDB)
    const stationToWrite = this.state.reservationStations.find(
      s => s.busy && s.executionTimeLeft === 0
    );

    if (!stationToWrite) {
      return; // Nenhum resultado para escrever neste ciclo
    }

    // Calcula o resultado
    let result: number | null = null;
    switch (stationToWrite.op) {
      case Opcode.ADD:
        result = (stationToWrite.vj ?? 0) + (stationToWrite.vk ?? 0);
        break;
      case Opcode.SUB:
        result = (stationToWrite.vj ?? 0) - (stationToWrite.vk ?? 0);
        break;
      case Opcode.MUL:
        result = (stationToWrite.vj ?? 0) * (stationToWrite.vk ?? 0);
        break;
      case Opcode.DIV:
        result = (stationToWrite.vk === 0) ? Infinity : (stationToWrite.vj ?? 0) / (stationToWrite.vk ?? 0);
        break;
      case Opcode.LW:
        result = this.state.memory[stationToWrite.address ?? -1] ?? 0; // Padrão para 0 se o endereço for inválido
        break;
      case Opcode.SW:
        if (stationToWrite.address !== null) {
          // Para SW, o valor a ser armazenado foi colocado em Vk durante a emissão
          this.state.memory[stationToWrite.address] = stationToWrite.vk ?? 0;
        }
        break;
    }

    const stationName = stationToWrite.name;

    // Transmite o resultado no CDB (se não for um SW)
    if (stationToWrite.op !== Opcode.SW && result !== null) {
      // Atualiza o arquivo de registradores
      if (stationToWrite.dest && this.state.registerFile[stationToWrite.dest]?.qi === stationName) {
        this.state.registerFile[stationToWrite.dest].value = result;
        this.state.registerFile[stationToWrite.dest].qi = null;
      }

      // Atualiza todas as estações de reserva que esperam por este resultado
      for (const station of this.state.reservationStations) {
        if (station.qj === stationName) {
          station.vj = result;
          station.qj = null;
        }
        if (station.qk === stationName) {
          station.vk = result;
          station.qk = null;
        }
      }
    }

    // Libera a estação de reserva
    stationToWrite.busy = false;
    stationToWrite.op = null;
    stationToWrite.vj = null;
    stationToWrite.vk = null;
    stationToWrite.qj = null;
    stationToWrite.qk = null;
    stationToWrite.address = null;
    stationToWrite.dest = null;
    stationToWrite.executionTimeLeft = null;
    stationToWrite.address = null;

    // Atualiza o status da instrução
    const instruction = this.instructionStatus.find(s => s.executionEnd !== null && s.writeResult === null)?.instruction;
    if (instruction) {
        const instructionStatus = this.instructionStatus.find(s => s.instruction === instruction && s.writeResult === null);
        if (instructionStatus) {
            instructionStatus.writeResult = this.state.clock;
        }
    }
  }

  /**
   * Retorna o estado atual da simulação.
   */
  public getState(): SimulationState {
    // Retorna uma cópia profunda para garantir a imutabilidade no lado do cliente (React)
    return JSON.parse(JSON.stringify(this.state));
  }

  public getInstructionStatus(): InstructionStatus[] {
    return JSON.parse(JSON.stringify(this.instructionStatus));
  }
}
